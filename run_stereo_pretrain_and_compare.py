"""
Run Stereo-Aware Pretraining and Comparison

1. Pretrain on ZINC with stereoisomer expansion (21 features)
2. Fine-tune on BBBP
3. Compare against baseline and current best (0.8316)
"""
...
def pretrain_stereo_encoder(num_zinc_molecules=50000, epochs=30, device='cpu'):
    ...
    if os.path.exists(save_path):
        print(f"Found existing pretrained model at {save_path}")
        user_input = input("Use existing? (y/n): ").strip().lower()
        if user_input == 'y':
            return save_path

    print("=" * 70)
    print("PHASE 1: PRETRAINING ON ZINC WITH STEREOISOMERS")
    print("=" * 70)
    sys.stdout.flush()

    # Load ZINC
    zinc_smiles = load_zinc_subset(num_zinc_molecules)
    if not zinc_smiles:
        print("ERROR: Could not load ZINC dataset")
        return None

    # Expand with stereoisomers
    print("\nExpanding ZINC with stereoisomers...")
    expanded_smiles = expand_zinc_with_stereoisomers(zinc_smiles, max_isomers_per_mol=4)

    # Convert to graphs
    print("\nConverting to graphs (21 features: 15 atomic + 6 stereo)...")
    sys.stdout.flush()
    ...
    from zinc_stereo_pretraining import count_stereocenters
    ...
    # Train
    loader = DataLoader(valid_graphs, batch_size=64, shuffle=True)
...
def main():
    print("=" * 70)
    print("BBB PREDICTION - STEREO-AWARE PRETRAINING COMPARISON")
    print("Goal: Beat current best of 0.8316 using proper stereo pretraining")
    print("=" * 70)
    ...
    # ========================================================================
    # PHASE 1: PRETRAIN ON ZINC WITH STEREOISOMERS
    # ========================================================================
    pretrained_path = pretrain_stereo_encoder(
        num_zinc_molecules=50000,  # Use 50k for faster testing
        epochs=30,
        device=device
    )
    ...
    torch.save(model_baseline.state_dict(), 'models/best_baseline_stereo_pretrain.pth')
    ...
    torch.save(model_stereo.state_dict(), 'models/best_stereo_only_pretrain.pth')
